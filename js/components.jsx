...
// ---------- Store lintas peran (dipakai saat satu peran memicu tindak lanjut di peran lain) ----------
window.__AIS_STORE = window.__AIS_STORE || {};
function useSharedList(key, seed) {
  if (!(key in window.__AIS_STORE)) window.__AIS_STORE[key] = seed;
  const [, force] = useState(0);
  useEffect(() => {
    const h = () => force((n) => n + 1);
    window.addEventListener("ais-store:" + key, h);
    return () => window.removeEventListener("ais-store:" + key, h);
  }, [key]);
  const setList = (updater) => {
    window.__AIS_STORE[key] = typeof updater === "function" ? updater(window.__AIS_STORE[key]) : updater;
    window.dispatchEvent(new CustomEvent("ais-store:" + key));
  };
  return [window.__AIS_STORE[key], setList];
}
...
function Icon({ name, size = 20, className = "", style = {} }) {
  ...
  const paths = {
    ...
    star: <><path {...P} d="m12 3 2.7 5.6 6.3.9-4.5 4.3 1 6.2-5.5-3-5.5 3 1-6.2L3 9.5l6.3-.9z" /></>,
    heart: <><path {...P} d="M12 20s-7-4.5-7-10a4 4 0 0 1 7-2.6A4 4 0 0 1 19 10c0 5.5-7 10-7 10Z" /></>,
    share: <><circle {...P} cx="6" cy="12" r="2.5" /><circle {...P} cx="18" cy="6" r="2.5" /><circle {...P} cx="18" cy="18" r="2.5" /><path {...P} d="m8.2 10.8 7.6-3.6M8.2 13.2l7.6 3.6" /></>,
    tag: <><path {...P} d="M3 12V5a2 2 0 0 1 2-2h7l9 9-9 9z" /><circle cx="8" cy="8" r="1.4" fill="currentColor" /></>,
    arrowR: <><path {...P} d="M4 12h16m0 0-6-6m6 6-6 6" /></>,
  };
  return (
