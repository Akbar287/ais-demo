...
function PMBDashboard({ nav }) {
  const R = window.AIS_ROLES; const p = R.personas.pmb;
  const funnel = [["Pendaftar", 3214, "blue"], ["Berkas Terverifikasi", 2890, "purple"], ["Ikut Tes", 2640, "amber"], ["Lulus Seleksi", 980, "green"], ["Daftar Ulang", 742, "orange"]];
    ...
    <div className="anim-in">
      ...
      <div className="card">
        ...
        <div className="card-pad" style={{ display: "grid", gap: 14 }}>
          {funnel.map(([l, v, t], i) => (
            <div key={l}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6, fontSize: 13.5 }}><span style={{ fontWeight: 600 }}>{i + 1}. {l}</span><b className="mono">{v.toLocaleString("id")}</b></div>
...
function PMBPendaftar() {
  const [q, setQ] = useState(""); const [f, setF] = useState("semua"); const [detail, setDetail] = useState(null);
...
Object.assign(window, { PMBDashboard, PMBPendaftar, PMBTes, PMBHasil, PMBDaftarUlang });
