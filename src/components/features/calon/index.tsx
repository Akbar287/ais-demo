...
function cbtLoad() {
  try {
    const storedState = localStorage.getItem(CBT_KEY);
    return storedState ? JSON.parse(storedState) : null;
  } catch {
    return null;
  }
}
function cbtSave(s) { try { localStorage.setItem(CBT_KEY, JSON.stringify(s)); } catch {} }
...
function CalonDashboard({ nav }) {
  const R = AIS_ROLES; const p = R.personas.calon;
  const saved = cbtLoad();
          ...
          <div style={{ padding: "6px 0" }}>
            {[
              ...
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 13, padding: "13px 24px", borderBottom: i < arr.length - 1 ? "1px solid var(--line)" : "none" }}>
                ...
                <Badge tone={typeof tone === "string" ? tone : "gray"} dot={tone !== "gray"}>{s}</Badge>
              </div>
            ))}
          </div>
        </div>
    ...
    <div className="anim-in">
      ...
      <div className="card" style={{ maxWidth: 620, overflow: "hidden" }}>
        <div style={{ background: "linear-gradient(115deg, var(--blue), color-mix(in srgb, var(--blue) 62%, #000))", color: "#fff", padding: "22px 26px", display: "flex", alignItems: "center", gap: 14 }}>
          <div className="sb-logo" style={{ width: 44, height: 44, fontSize: 20, background: "rgba(255,255,255,.18)" }}>A</div>
