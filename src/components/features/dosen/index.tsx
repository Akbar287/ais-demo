...
function buatRoster(prefix, n, seed) {
  const out: Array<{ no: number; nim: string; nama: string }> = [];
  for (let i = 0; i < n; i++) {
...
function DosenTA() {
  const toast = useToast();
  const [detail, setDetail] = useState<(typeof BIMBINGAN_TA)[number] | null>(null);
  return (
    <div className="anim-in">
...
export { DosenDashboard, DosenKelas, DosenPresensi, DosenNilai, DosenJadwal, DosenTA, ProfilStaff, buatRoster, KELAS_DIAMPU, BIMBINGAN_TA };
