...
function PAValidasi() {
  ...
  const [detail, setDetail] = useState<(typeof MHS_BIMBINGAN)[number] | null>(null);
  const menunggu = list.filter((m) => m.krs === "menunggu");

  function act(nim, status) {
    setList((l) => l.map((m) => m.nim === nim ? { ...m, krs: status } : m));
    setDetail(null);
    toast(status === "disetujui" ? "KRS disetujui & dikirim ke Enrollment Service" : "KRS dikembalikan ke mahasiswa", status === "disetujui" ? "ok" : "err");
  }
...
function PAMonitoring() {
  const byAngk: Record<number, typeof MHS_BIMBINGAN> = {};
...
export { PADashboard, PAValidasi, PAMahasiswa, PAMonitoring, MHS_BIMBINGAN };
