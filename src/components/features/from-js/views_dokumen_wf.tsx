/* eslint-disable */
// @ts-nocheck
// Generated from js/views_dokumen_wf.jsx by scripts/port-js-ssot.mjs.
"use client";
...
function DokDashboard({ nav }) {
  const R = window.AIS_ROLES; const p = R.personas.dokumen;
...
function WfDashboard({ nav }) {
  const R = window.AIS_ROLES; const p = R.personas.workflow;
  const tiles = [
...
function WfInbox() {
  ...
  function act(kode, st) { setList((l) => l.map((a) => a.kode === kode ? { ...a, st } : a)); setDetail(null); toast(st === "approved" ? "Disetujui — alur dilanjutkan ke tahap berikutnya" : "Ditolak — dikembalikan ke pemohon", st === "approved" ? "ok" : ""); }
...
export { DokDashboard, DokPusat, DokTTE, DokVerifikasi, WfDashboard, WfInbox, WfAlur };
