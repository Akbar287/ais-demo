/* eslint-disable */
// @ts-nocheck
// Generated from js/views_mbkm.jsx by scripts/port-js-ssot.mjs.
"use client";
...
function MbkmDashboard({ nav }) {
  const R = window.AIS_ROLES; const p = R.personas.mbkm;
...
// Tabel pendaftar berbagi store dgn sisi mahasiswa (MBKM & KKN).
// Tindak lanjut: daftar/seleksi → diterima/ditolak → berjalan → selesai (+konversi SKS).
function ExpPendaftar({ jenis, judul, desc }) {
  const E = window.AIS_EXP;
  const toast = useToast();
  const [list, setList] = useSharedList("exp_pendaftaran", E.seed.slice());
  const [nilaiFor, setNilaiFor] = useState(null);
  const [nilai, setNilai] = useState("A");
  const rows = list.filter((x) => x.jenis === jenis);

  function set(id, status, extra) { setList((l) => l.map((x) => x.id === id ? { ...x, status, ...(extra || {}) } : x)); }
  function terima(id) { set(id, "diterima"); toast("Pendaftar diterima — kontrak/penempatan disiapkan", "ok"); }
  function tolak(id) { set(id, "ditolak"); toast("Pendaftaran ditolak & dikembalikan ke mahasiswa", ""); }
  function mulai(id) { set(id, "berjalan"); toast("Kegiatan dimulai — mahasiswa dapat mengisi logbook", "ok"); }
  function selesaikan() { set(nilaiFor.id, "selesai", { nilai }); setNilaiFor(null); toast("Selesai — nilai & konversi " + nilaiFor.sks + " SKS dikirim ke transkrip (asm) & PDDikti", "ok"); }

  const counts = E.STATUS_FLOW.map((s) => rows.filter((x) => x.status === s).length);

  return (
    <div className="anim-in">
      <PageHead title={judul} desc={desc} />
      <div className="card card-pad" style={{ marginBottom: 16, display: "flex", gap: 18, flexWrap: "wrap", alignItems: "center" }}>
        {[["Mendaftar", counts[0] + counts[1], "amber"], ["Berjalan", counts[3], "blue"], ["Selesai", counts[4], "green"]].map(([l, v, t]) => (
          <div key={l} style={{ display: "flex", alignItems: "center", gap: 8 }}><span className="dot" style={{ background: `var(--${t})`, width: 9, height: 9 }} /><span style={{ fontSize: 13 }}><b>{v}</b> <span style={{ color: "var(--ink-3)" }}>{l}</span></span></div>
        ))}
        <span style={{ marginLeft: "auto", fontSize: 12.5, color: "var(--ink-3)", display: "flex", alignItems: "center", gap: 7 }}><Icon name="link" size={15} /> Terhubung langsung dgn pendaftaran mahasiswa</span>
      </div>
      <div className="card">
        <div style={{ overflowX: "auto" }}>
          <table className="tbl">
            <thead><tr><th>Mahasiswa</th><th>Program / Lokasi</th><th style={{ textAlign: "center" }}>IPK</th><th style={{ textAlign: "center" }}>SKS</th><th style={{ textAlign: "center" }}>Logbook</th><th>Status</th><th>Tindak Lanjut</th></tr></thead>
            <tbody>
              {rows.length === 0 && <tr><td colSpan={7} style={{ textAlign: "center", padding: "36px 0", color: "var(--ink-3)" }}>Belum ada pendaftar.</td></tr>}
              {rows.map((x) => (
                <tr key={x.id}>
                  <td><div style={{ fontWeight: 700 }}>{x.nama}</div><div className="mono" style={{ fontSize: 11.5, color: "var(--ink-3)" }}>{x.nim} · {x.prodi}</div></td>
                  <td style={{ maxWidth: 240, color: "var(--ink-2)" }}>{x.program}</td>
                  <td style={{ textAlign: "center", fontWeight: 800, fontFamily: "var(--mono)", color: x.ipk < 3 ? "var(--amber)" : "var(--ink)" }}>{(x.ipk || 0).toFixed(2)}</td>
                  <td style={{ textAlign: "center", fontFamily: "var(--mono)" }}>{x.sks}</td>
                  <td style={{ textAlign: "center", color: "var(--ink-3)" }}>{x.logbook ? x.logbook.length : 0}</td>
                  <td><Badge tone={E.STATUS_LABEL[x.status][1]} dot>{E.STATUS_LABEL[x.status][0]}</Badge>{x.nilai ? <Badge tone="green" style={{ marginLeft: 6 }}>Nilai {x.nilai}</Badge> : null}</td>
                  <td>
                    {(x.status === "daftar" || x.status === "seleksi") && (
                      <div style={{ display: "flex", gap: 6 }}>
                        <button className="btn btn-ghost btn-sm" onClick={() => tolak(x.id)}>Tolak</button>
                        <button className="btn btn-soft btn-sm" onClick={() => terima(x.id)}><Icon name="check" size={13} /> Terima</button>
                      </div>
                    )}
                    {x.status === "diterima" && <button className="btn btn-soft btn-sm" onClick={() => mulai(x.id)}>Mulai Kegiatan</button>}
                    {x.status === "berjalan" && <button className="btn btn-soft btn-sm" onClick={() => { setNilaiFor(x); setNilai("A"); }}>Selesaikan &amp; Nilai</button>}
                    {(x.status === "selesai" || x.status === "ditolak") && <span style={{ fontSize: 12, color: "var(--ink-3)" }}>—</span>}
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {nilaiFor && (
        <Modal title="Selesaikan & Konversi SKS" subtitle={nilaiFor.nama + " · " + nilaiFor.program} onClose={() => setNilaiFor(null)}
          footer={<><button className="btn btn-ghost" onClick={() => setNilaiFor(null)}>Batal</button><button className="btn btn-primary" onClick={selesaikan}><Icon name="check" size={16} /> Finalisasi &amp; Kirim ke Transkrip</button></>}>
          <div style={{ display: "flex", gap: 20, marginBottom: 16, flexWrap: "wrap" }}>
            {[["Program", nilaiFor.program], ["SKS dikonversi", nilaiFor.sks + " SKS"], ["Logbook", (nilaiFor.logbook ? nilaiFor.logbook.length : 0) + " entri"]].map(([l, v]) => (<div key={l}><div style={{ fontSize: 11, color: "var(--ink-3)", fontWeight: 600 }}>{l}</div><div style={{ fontWeight: 700, fontSize: 13.5, marginTop: 2 }}>{v}</div></div>))}
          </div>
          <div className="field" style={{ margin: 0 }}><label>Nilai Akhir (Huruf Mutu)</label>
            <select value={nilai} onChange={(e) => setNilai(e.target.value)}>{["A", "AB", "B", "BC", "C"].map((n) => <option key={n}>{n}</option>)}</select>
          </div>
          <div style={{ fontSize: 11.5, color: "var(--ink-3)", display: "flex", alignItems: "center", gap: 6, marginTop: 10 }}><Icon name="info" size={13} /> Finalisasi mempublikasikan event konversi SKS → transkrip (asm) & pelaporan PDDikti (fdr).</div>
        </Modal>
      )}
    </div>
  );
...
function MbkmPendaftar() {
  return <ExpPendaftar jenis="MBKM" judul="Pendaftar MBKM" desc="Seleksi & tindak lanjut pendaftar dari mahasiswa: terima/tolak → mulai → selesai. Penyelesaian memicu konversi SKS ke transkrip." />;
}

function KknPendaftar() {
  return <ExpPendaftar jenis="KKN" judul="Pendaftar KKN" desc="Seleksi & tindak lanjut pendaftar KKN dari mahasiswa: terima/tolak → penempatan/berjalan → penilaian & konversi SKS." />;
}
...
function KknDashboard({ nav }) {
  const R = window.AIS_ROLES; const p = R.personas.kkn;
...
export { MbkmDashboard, MbkmProgram, MbkmPendaftar, MbkmKonversi, MbkmMitra, KknDashboard, KknPeriode, KknKelompok, KknPenilaian, KknPendaftar };
