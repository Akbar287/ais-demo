/* eslint-disable */
// @ts-nocheck
// Generated from js/views_perpustakaan.jsx by scripts/port-js-ssot.mjs.
"use client";
...
// seed reservasi (18.5.2) — disimpan di store lintas peran window.__AIS_STORE.lib_reservasi
const LIB_RESERVASI_SEED = [
  { id: 1, judul: "Database System Concepts", anggota: "Bayu Pratama", nim: "11210910000033", tgl: "21 Jun 2026", status: "menunggu" },
];
...
function LibDashboard({ nav }) {
  ...
  const [reservasi] = useSharedList("lib_reservasi", LIB_RESERVASI_SEED.slice());
  const rsvMenunggu = reservasi.filter((r) => r.status === "menunggu" || r.status === "siap").length;
  const tiles = [
    ...
    { label: "Reservasi Menunggu", value: rsvMenunggu, meta: "dari Perpustakaan Saya", ic: "bell", c: "var(--amber)", bg: "var(--amber-bg)" },
  ];
  return (
    <div className="anim-in">
      <StaffHero persona={p} role="pustakawan" sub="Layani sirkulasi koleksi, kelola katalog, keanggotaan, dan repositori karya ilmiah."
        action={<button className="btn btn-primary" onClick={() => nav("lib_reservasi")}><Icon name="bell" size={17} /> Reservasi ({rsvMenunggu})</button>} />
...
function PerpusMahasiswa() {
  ...
  const toast = useToast();
  const [q, setQ] = useState("");
  const [reservasi, setReservasi] = useSharedList("lib_reservasi", LIB_RESERVASI_SEED.slice());
  const mhs = (window.AIS_DATA.mahasiswa) || { nama: "Aisyah Nur Ramadhani", nim: "11210910000045" };
  const list = LIB_KOLEKSI.filter((k) => k.judul.toLowerCase().includes(q.toLowerCase()) || k.penulis.toLowerCase().includes(q.toLowerCase()));
  const myReservasi = reservasi.filter((r) => r.nim === mhs.nim);
  function pesan(k) {
    setReservasi((r) => [...r, { id: Date.now(), judul: k.judul, anggota: mhs.nama, nim: mhs.nim, tgl: "24 Jun 2026", status: "menunggu" }]);
    toast("Buku direservasi \u2014 diteruskan ke Perpustakaan", "ok");
  }
  const RSV_ST = { menunggu: ["Menunggu", "amber"], siap: ["Siap Diambil", "green"], selesai: ["Selesai", "gray"], batal: ["Dibatalkan", "red"] };
  return (
    <div className="anim-in">
      ...
      {myReservasi.length > 0 && (
        <div className="card" style={{ marginBottom: 18 }}>
          <div className="card-head"><Icon name="bell" size={19} style={{ color: "var(--amber)" }} /><h3>Reservasi Buku Saya</h3></div>
          <div style={{ overflowX: "auto" }}>
            <table className="tbl">
              <thead><tr><th>Judul</th><th>Tgl Reservasi</th><th>Status</th><th></th></tr></thead>
              <tbody>{myReservasi.map((r) => (
                <tr key={r.id}>
                  <td style={{ fontWeight: 700 }}>{r.judul}</td>
                  <td style={{ fontSize: 13 }}>{r.tgl}</td>
                  <td><Badge tone={RSV_ST[r.status][1]} dot>{RSV_ST[r.status][0]}</Badge></td>
                  <td>{(r.status === "menunggu" || r.status === "siap") && <button className="btn btn-ghost btn-sm" style={{ color: "var(--red)" }} onClick={() => setReservasi((l) => l.map((x) => x.id === r.id ? { ...x, status: "batal" } : x))}>Batalkan</button>}</td>
                </tr>
              ))}</tbody>
            </table>
          </div>
      ...
      <div className="card">
        <div style={{ overflowX: "auto" }}>
          <table className="tbl">
            ...
            <tbody>{list.map((k) => {
              const mine = myReservasi.find((r) => r.judul === k.judul && (r.status === "menunggu" || r.status === "siap"));
              return (<tr key={k.kode}><td style={{ fontWeight: 700 }}>{k.judul}</td><td style={{ color: "var(--ink-2)" }}>{k.penulis}</td><td style={{ textAlign: "center" }} className="mono">{k.tersedia}/{k.eks}</td><td>{mine ? <Badge tone={RSV_ST[mine.status][1]} dot>{RSV_ST[mine.status][0]}</Badge> : k.tersedia > 0 ? <button className="btn btn-ghost btn-sm" onClick={() => pesan(k)}>Pesan</button> : <button className="btn btn-soft btn-sm" onClick={() => pesan(k)}>Reservasi Antre</button>}</td></tr>);
            })}</tbody>
          </table>
        </div>
      </div>
...
// ---------- Reservasi Buku (sisi pustakawan — tindak lanjut booking mahasiswa) ----------
function LibReservasi() {
  const toast = useToast();
  const [list, setList] = useSharedList("lib_reservasi", LIB_RESERVASI_SEED.slice());
  const RSV_ST = { menunggu: ["Menunggu", "amber"], siap: ["Siap Diambil", "green"], selesai: ["Selesai", "gray"], batal: ["Dibatalkan", "red"] };
  function siapkan(id) { setList((l) => l.map((r) => r.id === id ? { ...r, status: "siap" } : r)); toast("Buku disiapkan di loket — notifikasi terkirim ke anggota", "ok"); }
  function selesai(id) { setList((l) => l.map((r) => r.id === id ? { ...r, status: "selesai" } : r)); toast("Reservasi selesai — buku diserahkan", "ok"); }
  function batalkan(id) { setList((l) => l.map((r) => r.id === id ? { ...r, status: "batal" } : r)); toast("Reservasi dibatalkan", ""); }
  const aktif = list.filter((r) => r.status === "menunggu" || r.status === "siap");
  const arsip = list.filter((r) => r.status === "selesai" || r.status === "batal");
  return (
    <div className="anim-in">
      <PageHead title="Reservasi Buku" desc="Antrean reservasi/booking buku dari anggota (mahasiswa/dosen) — 18.5.2. Booking dari Perpustakaan Saya masuk otomatis ke sini." />
      <div className="card" style={{ marginBottom: 18 }}>
        <div className="card-head"><Icon name="clock" size={19} style={{ color: "var(--amber)" }} /><h3>Menunggu Tindak Lanjut</h3><Badge tone="amber" style={{ marginLeft: "auto" }}>{aktif.length}</Badge></div>
        {aktif.length ? (
          <div style={{ overflowX: "auto" }}><table className="tbl">
            <thead><tr><th>Judul</th><th>Anggota</th><th>Tgl Reservasi</th><th>Status</th><th></th></tr></thead>
            <tbody>{aktif.map((r) => (<tr key={r.id}>
              <td style={{ fontWeight: 700 }}>{r.judul}</td>
              <td><div>{r.anggota}</div><div className="mono" style={{ fontSize: 11.5, color: "var(--ink-3)" }}>{r.nim}</div></td>
              <td style={{ fontSize: 13 }}>{r.tgl}</td>
              <td><Badge tone={RSV_ST[r.status][1]} dot>{RSV_ST[r.status][0]}</Badge></td>
              <td><div style={{ display: "flex", gap: 6 }}>
                {r.status === "menunggu" && <button className="btn btn-soft btn-sm" onClick={() => siapkan(r.id)}>Siapkan</button>}
                {r.status === "siap" && <button className="btn btn-soft btn-sm" onClick={() => selesai(r.id)}>Serahkan</button>}
                <button className="btn btn-ghost btn-sm" style={{ color: "var(--red)" }} onClick={() => batalkan(r.id)}>Batalkan</button>
              </div></td>
            </tr>))}</tbody>
          </table></div>
        ) : <div className="empty" style={{ padding: "36px 20px" }}><Icon name="check" size={26} style={{ color: "var(--green)", marginBottom: 8 }} /><div style={{ color: "var(--ink-3)", fontSize: 13 }}>Tidak ada reservasi menunggu.</div></div>}
      </div>
      {arsip.length > 0 && (
        <div className="card">
          <div className="card-head"><Icon name="doc" size={19} style={{ color: "var(--ink-3)" }} /><h3>Riwayat</h3></div>
          <div style={{ overflowX: "auto" }}><table className="tbl">
            <thead><tr><th>Judul</th><th>Anggota</th><th>Tgl</th><th>Status</th></tr></thead>
            <tbody>{arsip.map((r) => (<tr key={r.id}><td style={{ fontWeight: 700 }}>{r.judul}</td><td>{r.anggota}</td><td style={{ fontSize: 13 }}>{r.tgl}</td><td><Badge tone={RSV_ST[r.status][1]} dot>{RSV_ST[r.status][0]}</Badge></td></tr>))}</tbody>
          </table></div>
        </div>
      )}
    </div>
...
export { LibDashboard, LibSirkulasi, LibKatalog, LibAnggota, LibDenda, LibRepo, PerpusMahasiswa, LibReservasi };
