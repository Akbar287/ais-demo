/* eslint-disable */
// @ts-nocheck
// Generated from js/views_kemahasiswaan.jsx by scripts/port-js-ssot.mjs.
"use client";
...
// ============================================================
const ORMAWA = [
  { nama: "BEM ITI", jenis: "Eksekutif", ketua: "Galang Maulana", anggota: 48, dana: 45000000, status: "Aktif" },
...
function KmhsDashboard({ nav }) {
  const R = window.AIS_ROLES; const p = R.personas.kemahasiswaan;
  const tiles = [
        ...
        <div className="card" style={{ alignSelf: "start" }}>
          ...
          <div className="card-pad" style={{ display: "grid", gap: 14 }}>
            {[["Bekerja", 198, "green"], ["Lanjut Studi", 24, "blue"], ["Wirausaha", 30, "purple"], ["Belum terlacak", 60, "amber"]].map(([l, v, t]) => (
              <div key={l}><div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, marginBottom: 5 }}><span style={{ fontWeight: 600 }}>{l}</span><b className="mono">{v}</b></div><div className="prog"><i style={{ width: (v / 312 * 100) + "%", background: `var(--${t})` }} /></div></div>
            ))}
          </div>
        </div>
      </div>
...
function KsDashboard({ nav }) {
  const R = window.AIS_ROLES; const p = R.personas.kerjasama;
...
export { KmhsDashboard, KmhsOrmawa, KmhsBeasiswa, KmhsTracer, KmhsKarir, KsDashboard, KsMou, KsMitra };
