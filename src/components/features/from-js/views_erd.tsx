/* eslint-disable */
// @ts-nocheck
// Generated from js/views_erd.jsx by scripts/port-js-ssot.mjs.
"use client";
...
import { EntityCrud } from "./views_erd_crud";
...
function AdminERD() {
  ...
  const [open, setOpen] = useState(null); // subject no
  const [q, setQ] = useState("");
  const [detail, setDetail] = useState(null); // entity object
  ...
  // ---------- Entity detail modal (L4 struktur + CRUD data) ----------
  const entModal = detail && <EntDetailModal detail={detail} onClose={() => setDetail(null)} />;
...
// Modal detail entitas: tab Struktur (L4) + Data (CRUD)
function EntDetailModal({ detail, onClose }) {
  const E = window.AIS_ERD;
  const [tab, setTab] = useState("struktur");
  const fields = E.fieldsByEnt[detail.kode] || [];
  return (
    <Modal wide title={detail.nama} subtitle={"Entitas " + detail.kode + " · " + detail.subjek} onClose={onClose}
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap", alignItems: "center" }}>
        ...
        <div className="seg" style={{ marginLeft: "auto" }}>
          <button className={tab === "struktur" ? "on" : ""} onClick={() => setTab("struktur")}>Struktur ({fields.length})</button>
          <button className={tab === "data" ? "on" : ""} onClick={() => setTab("data")}>Data (CRUD)</button>
        </div>
      </div>
      {detail.ket && tab === "struktur" && <div style={{ fontSize: 13, color: "var(--ink-2)", marginBottom: 12, lineHeight: 1.5 }}>{detail.ket}{detail.rel ? <span style={{ color: "var(--ink-3)" }}> · Relasi {detail.rel}</span> : null}</div>}
      {tab === "struktur" ? (
        <div style={{ border: "1px solid var(--line)", borderRadius: 12, overflow: "auto", maxHeight: 420 }}>
          <table className="tbl">
        ...
        <EntityCrud ent={detail} fields={fields} />
      )}
    </Modal>
  );
}

export { AdminERD, EntDetailModal };
