/* eslint-disable */
// @ts-nocheck
// Generated from js/views_tu_itsm.jsx by scripts/port-js-ssot.mjs.
"use client";
...
function TuDashboard({ nav }) {
  const R = window.AIS_ROLES; const p = R.personas.tu;
...
function ItsmDashboard({ nav }) {
  const R = window.AIS_ROLES; const p = R.personas.itsm;
...
function ItsmSLA() {
  const cats = [["Akses/IAM", 96, 38], ["Jaringan", 88, 52], ["Email", 92, 24], ["Perangkat", 81, 31], ["Software", 95, 19], ["Aplikasi", 90, 44]];
  return (
    <div className="anim-in">
        ...
        {list.map((a, i) => (
          <div key={i} className="card card-pad" style={{ display: "flex", alignItems: "center", gap: 14, cursor: "pointer" }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontSize: 12, color: "var(--ink-3)" }}>{a.kategori} · {a.dibaca.toLocaleString("id")} kali dibaca</div>
            </div>
...
export { TuDashboard, TuMasuk, TuKeluar, TuDisposisi, TuArsip, ItsmDashboard, ItsmTiket, ItsmSLA, ItsmKB };
