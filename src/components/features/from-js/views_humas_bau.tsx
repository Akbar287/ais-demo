/* eslint-disable */
// @ts-nocheck
// Generated from js/views_humas_bau.jsx by scripts/port-js-ssot.mjs.
"use client";
...
// ---------------- HUMAS ----------------
const BERITA = [
  { judul: "ITI Raih Akreditasi Unggul dari BAN-PT", kategori: "Prestasi", tgl: "22 Jun 2026", dibaca: 3420, status: "Terbit" },
...
function HumasDashboard({ nav }) {
  const R = window.AIS_ROLES; const p = R.personas.humas;
  const tiles = [
        ...
        <div>
          {BERITA.slice(0, 4).map((b, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, padding: "13px 22px", borderBottom: i < 3 ? "1px solid var(--line)" : "none" }}>
              <div style={{ flex: 1, minWidth: 0 }}><div style={{ fontWeight: 700, fontSize: 13.5 }}>{b.judul}</div><div style={{ fontSize: 12, color: "var(--ink-3)" }}>{b.kategori} · {b.tgl} · {b.dibaca.toLocaleString("id")} dibaca</div></div>
            ...
            <tbody>
              {BERITA.map((b, i) => (
                <tr key={i}>
                  ...
                  <td style={{ textAlign: "center", fontFamily: "var(--mono)" }}>{b.dibaca.toLocaleString("id")}</td>
...
const PEMINJAMAN_KEND = [
  { pemohon: "BEM ITI", kendaraan: "Bus Hino R260", keperluan: "Studi banding", tgl: "27 Jun 2026", status: "menunggu" },
...
function BauDashboard({ nav }) {
  const R = window.AIS_ROLES; const p = R.personas.bau;
  const tiles = [
...
export { HumasDashboard, HumasBerita, HumasPPID, HumasPengaduan, BauDashboard, BauKendaraan, BauRT };
