...
function KeuDashboard({ nav }) {
  const R = AIS_ROLES; const p = R.personas.keuangan;
...
function KeuValidasi() {
  const toast = useToast();
  const [list] = useState(
    TAGIHAN.filter((item) => item.status === "menunggu").map((item, index) => ({
      ...item,
      bank: ["BSI", "Mandiri", "BNI"][index % 3],
      waktu: `23 Jun 09:${10 + ((index * 7) % 40)}`,
    })),
  );
  const [done, setDone] = useState<Array<{ nim: string; ok: boolean }>>([]);
...
function KeuLaporan() {
  const bln: Array<[string, number]> = [["Jan", 2.1], ["Feb", 4.8], ["Mar", 1.2], ["Apr", 0.9], ["Mei", 0.6], ["Jun", 0.4], ["Jul", 0.3], ["Agu", 3.2], ["Sep", 5.4], ["Okt", 1.1], ["Nov", 0.7], ["Des", 0.5]];
...
export { KeuDashboard, KeuTagihan, KeuValidasi, KeuPiutang, KeuBeasiswa, KeuLaporan };
