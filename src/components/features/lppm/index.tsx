...
function LppmDashboard({ nav }) {
  const R = AIS_ROLES; const p = R.personas.lppm;
  const tiles = [
            ...
            {LPPM_PROPOSAL.filter((x) => x.tahap === "review").map((x, i, arr) => (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, padding: "13px 22px", borderBottom: i < arr.length - 1 ? "1px solid var(--line)" : "none" }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 12, color: "var(--ink-3)" }}>{x.ketua} · {x.skema} · {rupiahJuta(x.dana)}</div>
                </div>
        ...
        {LPPM_HIBAH.map((h) => (
          <div key={h.kode} className="card card-pad">
            ...
            <div style={{ display: "flex", gap: 20, marginBottom: 14 }}>
              <div><div style={{ fontSize: 11, color: "var(--ink-3)", fontWeight: 600 }}>Dana / Judul</div><div style={{ fontWeight: 800, fontFamily: "var(--mono)", fontSize: 15 }}>{rupiahJuta(h.dana)}</div></div>
...
function LppmProposal() {
  ...
  const [detail, setDetail] = useState<(typeof LPPM_PROPOSAL)[number] | null>(null);
  function decide(judul, tahap) { setList((l) => l.map((x) => x.judul === judul ? { ...x, tahap } : x)); setDetail(null); toast(tahap === "lolos" ? "Proposal disetujui untuk didanai" : tahap === "revisi" ? "Proposal diminta revisi" : "Proposal ditolak", tahap === "lolos" ? "ok" : "err"); }
  return (
    <div className="anim-in">
            ...
            <tbody>
              {list.map((x) => (
                <tr key={x.judul}>
                  ...
                  <td style={{ textAlign: "right", fontFamily: "var(--mono)", fontWeight: 700 }}>{rupiahJuta(x.dana)}</td>
      ...
      {detail && (
        <Modal title={detail.judul} subtitle={detail.ketua + " · " + detail.skema} onClose={() => setDetail(null)}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 16 }}>
            {[["Skema", detail.skema], ["Usulan Dana", rupiahJuta(detail.dana)], ["Reviewer", detail.reviewer], ["Skor", detail.skor ?? "Belum dinilai"]].map(([l, v]) => (
...
function LpmDashboard({ nav }) {
  const R = AIS_ROLES; const p = R.personas.lpm;
...
export {
  LppmDashboard, LppmHibah, LppmProposal, LppmLuaran, LppmHKI,
  LpmDashboard, LpmAudit, LpmAkreditasi, LpmTemuan, LpmStandar,
};
